<template>
  <div class="month-number heiInher">
    <img
      class="month-number-vein left"
      src="@/assets/image/titleAreaTexture.svg"
      alt=""
    />
    <img
      class="month-number-vein right"
      src="@/assets/image/titleAreaTexture.svg"
      alt=""
    />
    <p class="modlue-title">月度样品数量</p>
    <div class="modlue-date">
      <p class="date-name">{{ time }}</p>
      <img class="date-img" src="@/assets/image/icon/date.svg" alt="" />
    </div>
    <div class="modlue-cont" id="histogramNumber"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    drawHistogram() {
      ...
      const myChart = echarts.init(document.getElementById("histogramNumber"));
      const option = {
        grid: {
          top: "10%",
          left: "2%",
